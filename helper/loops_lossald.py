...
def train_distill(epoch, train_loader, module_list, criterion_list, optimizer, opt):
    ...
    for idx, data in enumerate(train_loader):
        ...
        num=10
        n = opt.n
        input_adv = ald_semi_loss(model_t, input[:num, ::], n)
        input_all = torch.cat([input_adv, input], dim=0)
        target_all = torch.cat([target[:num].repeat(n), target], dim=0)
        
        feat_s, logit_s_all = model_s(input_all, is_feat=True, preact=preact)
        logit_s = logit_s_all[n*num:, ::]
        feat_s = [f[n*num:, ::] for f in feat_s]

        feat_t, logit_t_all = model_t(input_all, is_feat=True, preact=preact)
        logit_t = logit_t_all[n*num:, ::]
        feat_t = [f[n*num:, ::].detach() for f in feat_t]
        ...
        # print(grad_loss)
        loss = opt.gamma * loss_cls + opt.alpha * loss_div + opt.beta * loss_kd + opt.ald*grad_loss
